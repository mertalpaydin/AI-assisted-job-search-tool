...
# Providers used when none are named. SerpApi and RapidAPI need paid keys (and
# Google for Jobs is suppressed in Germany), so they are opt-in.
DEFAULT_PROVIDERS: tuple[str, ...] = ("indeed", "arbeitsagentur")

# Match columns reset to NULL when a posting no longer matches a LinkedIn job,
# so a stale match from an earlier run does not survive.
_MATCH_FIELDS = (
...
class ExternalSearchOrchestrator:
            ...
            for provider in selected_providers:
                ...
                for job_dict in raw_jobs:
                    if should_stop and should_stop():
                        ...
                        break
                    # One malformed posting must not drop the rest of the batch.
                    try:
                        self._process_job(job_dict, stats)
                    except Exception as je:
                        stats["errors"] += 1
                        logger.warning(
                            "Skipping {} job '{}' ({}): {}",
                            provider.name, job_dict.get("title"), job_dict.get("external_id"), je,
                        )
    ...
    def _process_job(self, job_dict: dict[str, Any], stats: dict[str, Any]) -> None:
        """Prefilter, match against jobs.db and save one posting."""
        ...
        # 3. Match against LinkedIn jobs in jobs.db; clear any stale match
        match = None
        if job_dict["prefilter_status"] == "accepted":
            match = self.matcher.find_match(comp, title)
        if match:
            job_dict.update(match)
            stats["matched_linkedin"] += 1
        else:
            job_dict.update(dict.fromkeys(_MATCH_FIELDS))

        # 4. Save to external_jobs.db
        _, is_new = self.ext_db.upsert_job(job_dict, clear_match=match is None)
        if is_new:
